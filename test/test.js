  ...
  .addBatch({
    ...
    'with test14.ics (testing comma-separated exdates)': {
      ...
      'event with comma-separated exdate': {
        topic(events) {
          return _.select(_.values(events), x => x.uid === '98765432-ABCD-DCBB-999A-987765432123')[0];
        },
    ...
    'with test14.ics (testing exdates with bad times)': {
      ...
      'event with exdates with bad times': {
        topic(events) {
          return _.select(_.values(events), x => x.uid === '1234567-ABCD-ABCD-ABCD-123456789012')[0];
        },
    ...
    'with test18.ics (testing for detecting timezones)': {
      ...
      'event 1c943': {
        topic(events) {
          return _.select(_.values(events), x => x.uid === '1C9439B1-FF65-11D6-9973-003065F99D04')[0];
        },
        'datetype is date-time'(topic) {
          assert.equal(topic.datetype, 'date-time');
        },
      ...
      'event 2c943': {
        topic(events) {
          return _.select(_.values(events), x => x.uid === '2C9439B1-FF65-11D6-9973-003065F99D04')[0];
        },
        'datetype is date-time'(topic) {
          assert.equal(topic.datetype, 'date-time');
        },
      ...
      'event 3c943': {
        topic(events) {
          return _.select(_.values(events), x => x.uid === '3C9439B1-FF65-11D6-9973-003065F99D04')[0];
        },
        'datetype is date-time'(topic) {
          assert.equal(topic.datetype, 'date-time');
        },
      ...
      'event 4c943': {
        topic(events) {
          return _.select(_.values(events), x => x.uid === '4C9439B1-FF65-11D6-9973-003065F99D04')[0];
        },
        'datetype is date'(topic) {
          assert.equal(topic.datetype, 'date');
        },
      ...
      'event 5c943': {
        topic(events) {
          return _.select(_.values(events), x => x.uid === '5C9439B1-FF65-11D6-9973-003065F99D04')[0];
        },
        'datetype is date'(topic) {
          assert.equal(topic.datetype, 'date');
        },
    ...
    'with test21.ics': {
      ...
      'using an event containing a start date without time zone': {
        topic(events) {
          return _.select(_.values(events), x => x.uid === 'f683404f-aede-43eb-8774-27f62bb27c92')[0];
        },
    ...
    'with test_with_multiple_tzids_in_vtimezone.ics': {
      ...
      'using a vtimezone with multiple timezone': {
        topic(events) {
          return _.select(_.values(events), x => x.uid === '1891-1709856000-1709942399@www.washougal.k12.wa.us')[0];
        },
        'has a start'(topic) {
    ...
    'with test23.ics (testing dtstart of rrule with timezones)': {
      ...
      'first event': {
        topic(events) {
          return _.select(_.values(events), x => x.uid === '000021a')[0];
        },
        'datetype is date-time'(topic) {
          assert.equal(topic.datetype, 'date-time');
        },
      ...
      'recurring yearly first event (14 july)': {
        topic(events) {
          ...
          const ev = _.select(_.values(events), x => x.uid === '000021a')[0];
      ...
      'second event': {
        topic(events) {
          return _.select(_.values(events), x => x.uid === '000021b')[0];
        },
    ...
    'with germany_at_end_of_day_repeating.ics': {
      ...
      checkEnd: {
        topic(events) {
          return _.select(_.values(events), x => x.uid === '2m6mt1p89l2anl74915ur3hsgm@google.com')[0];
        },
        'task completed'(task) {
    ...
    'with whole_day_moved_over_dst_change_berlin.ics': {
      ...
      checkEnd: {
        topic(events) {
          return _.select(_.values(events), x => x.uid === '14nv8jl8d6dvdbl477lod4fftf@google.com')[0].recurrences['2024-10-28'];
        },
        'topic recurrence': {
